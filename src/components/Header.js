...
const Header = () => {
    ...
    function avatar() {
        return <div class="relative flex items-center justify-end w-1/4 p-1 mr-2  sm:mr-0 sm:right-auto">
            <p className="mr-3 font-semibold text-slate-100">{web3.account.substr(0,4)}...{web3.account.substr(web3.account.length - 3,web3.account.length)}</p>
            <a href="#" class="relative block">
                <img alt="profile" src={`https://avatars.dicebear.com/api/identicon/${web3?.account}.svg`} class="mx-auto object-cover rounded-full h-10 w-10 border-2" />
            </a>
        </div>;
    }
